...
from frequenz.channels import FastBroadcast, Receiver, Sender
...
async def benchmark_broadcast(
    ...
    """
    channels: List[FastBroadcast[int]] = [FastBroadcast("meter") for _ in range(num_channels)]
...
def run() -> None:
    ...
    with open("/dev/stdout", "w", encoding="utf-8") as csvfile:
        ...
        fields = run_one(component_sender_benchmark, 0, 0, 0, "", 0.0)
        out = csv.DictWriter(csvfile, list(fields.keys()))
        out.writeheader()
        # out.writerow(run_one(component_sender_benchmark, 1, 1, 1, "multiple", 0.2))
        ...
        out.writerow(
            run_one(benchmark_single_task_broadcast, 100, 10, 50, "single", 0.0)
        )
        out.writerow(
            run_one(benchmark_single_task_broadcast, 100, 10, 50, "single", 0.0)
        )
